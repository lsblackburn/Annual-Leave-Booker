...
def dashboard_view():
    ...
    leaves = AnnualLeave.query.filter(
        AnnualLeave.status == 'Approved',
        AnnualLeave.start_date.between(now, four_weeks_later)
    ).order_by(AnnualLeave.start_date.asc()).all()
...
@login_required
def your_leave_view():
    # Fetch all leave records for the current user, ordered by updated_at descending. 
